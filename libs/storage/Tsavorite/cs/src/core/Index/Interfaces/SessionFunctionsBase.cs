    ...
    public abstract class SessionFunctionsBase<TInput, TOutput, TContext> : ISessionFunctions<TInput, TOutput, TContext>
    {
        ...
        public virtual RecordFieldInfo GetUpsertFieldInfo<TKey>(TKey key, ReadOnlySpan<byte> value, ref TInput input)
            where TKey : IKey
                ...
                , allows ref struct
#endif
            => new() { KeySize = key.KeyBytes.Length, ValueSize = value.Length, ValueIsObject = false, ExtendedNamespaceSize = RecordFieldInfo.GetExtendedNamespaceSize(in key) };
        /// <inheritdoc/>
        public virtual RecordFieldInfo GetUpsertFieldInfo<TKey>(TKey key, IHeapObject value, ref TInput input)
            where TKey : IKey
                ...
                , allows ref struct
#endif
            => new() { KeySize = key.KeyBytes.Length, ValueSize = ObjectIdMap.ObjectIdSize, ValueIsObject = true, ExtendedNamespaceSize = RecordFieldInfo.GetExtendedNamespaceSize(in key) };
        /// <inheritdoc/>
...
#endif
            where TSourceLogRecord : ISourceLogRecord
            => new() { KeySize = key.KeyBytes.Length, ValueSize = inputLogRecord.DataHeader.ValueIsObject ? ObjectIdMap.ObjectIdSize : inputLogRecord.ValueSpan.Length, ValueIsObject = inputLogRecord.DataHeader.ValueIsObject, ExtendedNamespaceSize = RecordFieldInfo.GetExtendedNamespaceSize(in key) };

        /// <inheritdoc/>
        public virtual void ConvertOutputToHeap(ref TInput input, ref TOutput output) { }

        public virtual void PreSingleKeyConsistentRead(long hash) { }

        public virtual void PostSingleKeyConsistentReadCallback() { }

        public virtual void PreBatchKeyConsistentReadCallback(ReadOnlySpan<PinnedSpanByte> parameters) { }

        public virtual bool PostBatchKeyConsistentReadCallback(int keyCount) => true;
    }
}