--------------------- MODULE CasAnnounceSymmetricPeers ---------------------
(***************************************************************************)
...
\* Volatile.Write(slot.localCurrentEpoch, 0) — release store. The peer then
\* loops: Tsavorite session threads acquire and release repeatedly, so a peer
\* can re-enter protection at a newer epoch and run the scan on a later round,
\* after some other peer has retired an object.
ReleaseSlot(p) ==
    /\ pc[p] = "ReleaseSlot"
