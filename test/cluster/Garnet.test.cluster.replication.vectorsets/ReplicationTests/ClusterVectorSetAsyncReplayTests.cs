...
{
    /// <summary>
    /// Vector Set coverage for asynchronous AOF replay, i.e. <c>ReplicationOffsetMaxLag = -1</c>.
    ///
    /// With synchronous replay (lag 0) the replica applies each AOF batch inline on the receive
    /// path before the replication offset advances, so replay is serialized behind the network read.
    /// Asynchronous replay decouples them: the replica takes the batch, advances its AOF tail, and
    /// replays on background tasks, so replay runs concurrently with ingestion and may lag.
    ///
    /// Vector Sets need separate coverage because VADD mutates a native DiskANN index instead of
    /// just writing bytes, and those mutations are dispatched onto VectorManager replay tasks.
    /// </summary>
    [TestFixture]
    [NonParallelizable]
