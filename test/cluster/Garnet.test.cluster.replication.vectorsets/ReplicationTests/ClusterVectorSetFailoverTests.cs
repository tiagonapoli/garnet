...
{
    /// <summary>
    /// Failover coverage for populated Vector Sets. Promotion reverses the replication direction, so the
    /// promoted node must serve the set it previously only received, and the demoted node must accept it back.
    /// </summary>
    [TestFixture]
    [NonParallelizable]
