...
namespace Garnet.test
{
    ...
    /// <c>VectorManager.WaitForCleanupCompleteAsync</c> against a single server, proving the drain semantics
    /// that the cluster full-sync paths depend on.
    /// </summary>
    public abstract class VectorSetCleanupTestBase : TestBase
    {
        ...
        /// <summary>
        /// Blocking drain of the cleanup pipeline.
        /// </summary>
        protected static void DrainCleanup(VectorManager vectorManager) => AsyncUtils.BlockingWait(vectorManager.WaitForCleanupCompleteAsync());
        ...
        /// <summary>
        /// Wait for a background task to park on an armed injection seam. The seam clears its own flag on
        /// arrival, which is the "arrived, parked" signal; re-arming it releases the parked task.
        /// </summary>
