...
namespace Garnet.test
{
    ...
    public class VectorSetCleanupWorkChannelTests
    {
        ...
        [Test]
        public async Task WaitToAcquireAsyncYieldsItemsThenNullOnceCompleted()
        {
            var counter = new VectorSetCleanupWorkCounter();
            var channel = new VectorSetCleanupWorkChannel<int>(counter);
            ...
            var acquired = await channel.WaitToAcquireAsync();
            ClassicAssert.IsTrue(acquired.HasValue);
            ClassicAssert.AreEqual(1, acquired.Value.Item);

            acquired.Value.Dispose();
            ClassicAssert.AreEqual(0, counter.Inflight);

            channel.CompleteAndWaitForConsumerTask(Task.CompletedTask);
            ClassicAssert.IsNull(await channel.WaitToAcquireAsync());
        }
