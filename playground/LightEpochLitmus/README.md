...
only here. `--buggy` runs the harness against it, so the fixed and unfixed algorithms
can be compared on the same machine in the same session instead of by reverting the
...
`0`. A `--buggy` run that comes back clean means the machine is not producing the
window at all, and the clean result beside it proves nothing.
