...
`BuggyLightEpoch.cs` is a copy of `LightEpoch` as it stands on `main`, generated by
`regen-buggy-epoch.ps1` and used only here. `--buggy` runs the harness against it, so
the fixed and unfixed algorithms can be compared on the same machine in the same
session instead of by reverting the source and rebuilding:

```
dotnet run -c Release --project playground/LightEpochLitmus -- --buggy --seconds 20
...
Regenerate the copy after `main` moves:
